...
module SoundsHelper
    def image_for(sound, size)
        if sound.path
          ...
          thumb_path = "/uploads/sounds/" + sound.path.to_s + "/thumbnail/" +Settings.IMAGE_HEAD_NAME.to_s + sound.path.to_s + Settings.SMALL.to_s + sound.img_ext_name.to_s
    ...
    def audio_for(sound)
        if sound.path
          # image_tagを用いてuserのプロフィール画像を表示してください
          #normal_path = sound.img_path.to_s + sound.img_ext_name.to_s
          audio_path = "/uploads/sounds/" + sound.path.to_s + "/sound/" + Settings.SOUND_HEAD_NAME.to_s + sound.path.to_s + sound.ext_name.to_s
          audio_tag(audio_path, :preload => "auto", :controls => true, class: "audio")
          #↑class:でHTMLでのclassを設定できる。
          #↑thumb_pathはサムネイルのパス。
          #↑×の全角半角には気をつける。普通に入力すると全角。
        else
